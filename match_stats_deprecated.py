...
def scrape_match_stats(match_stats_url):
    ...
    tourney_id = match_stats_url_suffix.split('/')[6]
...
def scrape_match_stats_revised(match_stats_url):
    match_tree = html_parse_tree(match_stats_url)
    ...
    test_xpath = "//*[@id='RGMatchStats']"
    test_parsed = xpath_parse(match_tree, test_xpath)
    print(test_parsed)
        ...
        for j in range(0, tourney_round_count):
            ...
            for k in range(0, round_match_count):
                ...
                if len(match_stats_url_cleaned) > 0:
                    ...
                    # # # # # # # # # # # # # # # # # # #
                    if match_stats_url_suffix_split[3] == 'stats-centre': scraped_stats = scrape_match_stats_revised(match_stats_url)
                    else: scraped_stats = scrape_match_stats(match_stats_url)
