...
def generate_matchs(start_year, end_year, outputfile):
    # STEP 1: Scrape year page
    tourney_match = []
    tourney_match.append(["tourney_year_id", "tourney_order", "tourney_name", "tourney_slug", "tourney_url_suffix", "start_date", "start_year", "start_month", "start_day", "end_date", "end_year", "end_month", "end_day", "currency", "prize_money", "match_index", "tourney_round_name", "round_order", "match_order", "winner_name", "winner_player_id", "winner_slug", "loser_name", "loser_player_id", "loser_slug", "winner_seed", "loser_seed", "match_score_tiebreaks", "winner_sets_won", "loser_sets_won", "winner_games_won", "loser_games_won", "winner_tiebreaks_won", "loser_tiebreaks_won", "match_id", "match_stats_url_suffix"])
    for h in range(int(start_year), int(end_year) + 1):

        year = str(h)
        tourney_data_scrape, tourney_urls_scrape = scrape_year(year)
        ...
        for i in range(0 , len(tourney_urls_scrape)):
            if len(tourney_urls_scrape[i]) > 0:
                ...
                match_data_scrape, match_urls_scrape = scrape_tourney(tourney_urls_scrape[i])
            ...
            array2csv(tourney_match, outputfile)